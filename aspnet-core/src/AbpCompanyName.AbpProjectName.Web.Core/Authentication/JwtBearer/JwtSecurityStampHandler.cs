...
namespace AbpCompanyName.AbpProjectName.Authentication.JwtBearer
{
    public class JwtSecurityStampHandler : IJwtSecurityStampHandler, ITransientDependency
    {
        ...
        public async Task<bool> Validate(ClaimsPrincipal claimsPrincipal)
        {
            ...
            var securityStampKey = claimsPrincipal.Claims.FirstOrDefault(c => c.Type == AbpProjectNameConsts.SecurityStampKey);
            if (securityStampKey == null)
            {
                return false;
            }

            var userIdentifierString = claimsPrincipal.Claims.First(c => c.Type == AbpProjectNameConsts.UserIdentifier);
        ...
        {
            await _cacheManager.GetCache(AbpProjectNameConsts.SecurityStampKey)
        ...
        {
            await _cacheManager.GetCache(AbpProjectNameConsts.SecurityStampKey).RemoveAsync(GenerateCacheKey(tenantId, userId));
        }
        ...
        private async Task<bool> ValidateSecurityStampFromCache(UserIdentifier userIdentifier, string securityStamp)
        {
            var securityStampKey = await _cacheManager
                .GetCache(AbpProjectNameConsts.SecurityStampKey)
        ...
        private async Task<bool> ValidateSecurityStampFromDb(UserIdentifier userIdentifier, string securityStamp)
        {
            using (var uow = _unitOfWorkManager.Begin())
            {
                ...
                {
                    var user = await _userManager.GetUserByIdAsync(userIdentifier.UserId);
